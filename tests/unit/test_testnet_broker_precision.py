...
def test_quantize_qty_uses_market_lot_size_for_market_orders() -> None:
    ...
    sent, meta = broker.quantize_qty(symbol="BTCUSDT", qty="24.087679152113694", is_market=True, reference_price="150")
    assert sent == "24"
...
def test_quantize_qty_uses_lot_size_for_non_market_orders() -> None:
    broker = _stub_broker(lot_step="0.01", market_step="0.1")
    sent, meta = broker.quantize_qty(symbol="BTCUSDT", qty="24.087679152113694", is_market=False, reference_price="150")
    assert sent == "24.08"
...
def test_regression_high_precision_qty_becomes_valid_step_multiple() -> None:
    ...
    sent, _ = broker.quantize_qty(symbol="BTCUSDT", qty="24.087679152113694", is_market=True, reference_price="100")
    sent_d = Decimal(sent)
    step = Decimal("0.001")
...
def test_quantize_price_uses_tick_size() -> None:
    ...
    sent, meta = broker.quantize_price(symbol="BTCUSDT", price="154.189")
    assert sent == "154.18"
...
def test_quantize_qty_rejects_when_below_min_notional() -> None:
    ...
    with pytest.raises(OrderValidationError, match="minNotional"):
        broker.quantize_qty(symbol="BTCUSDT", qty="24.087679152113694", is_market=True, reference_price="0.1")


def test_quantize_qty_rejects_when_floored_to_zero() -> None:
    broker = _stub_broker(lot_step="0.001", market_step="0.001", min_notional=None)
    with pytest.raises(OrderValidationError, match="floored to 0"):
        broker.quantize_qty(symbol="BTCUSDT", qty="0.0004", is_market=True, reference_price="100")
...
def test_get_algo_open_orders_wires_expected_endpoint() -> None:
    ...
    def _fake_request(method: str, path: str, *, params=None, signed=False, timeout: float = 10.0):
        ...
        return [{"algoId": 123, "symbol": "BTCUSDT", "status": "NEW"}]
...
def test_place_market_order_sends_new_client_order_id() -> None:
    ...
    out = broker.place_market_order(symbol="BTCUSDT", side="BUY", quantity=1.0, reference_price=100.0)
...
def test_place_algo_orders_send_client_algo_ids() -> None:
    ...
    broker.place_take_profit_market(symbol="BTCUSDT", side="SELL", quantity=1.0, stop_price=101.0)
    broker.place_stop_market(symbol="BTCUSDT", side="SELL", quantity=1.0, stop_price=99.0)
...
def test_get_order_accepts_orig_client_order_id() -> None:
    ...
    out = broker.get_order(symbol="BTCUSDT", client_order_id="cid-1")
    assert out["orderId"] == 4321
    assert observed["method"] == "GET"
    assert observed["path"] == "/fapi/v1/order"
    assert observed["params"] == {"symbol": "BTCUSDT", "origClientOrderId": "cid-1"}
    assert observed["signed"] is True
