...
from tests.helpers import LONG_ENTRY_SIGNAL, SHORT_ENTRY_SIGNAL, build_test_execution_specs
...
def _build_strategy_result(df_raw: pd.DataFrame, *, signal: int) -> StrategyBuildResult:
    df_sig = df_raw.copy()
...
def _seed_open_position_state(run_dir: Path, *, side: str, qty: float, symbol: str = "BTCUSDT") -> None:
...
def test_runtime_reconciliation_blocks_new_entry_and_emits_runtime_reason(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: _build_strategy_result(kwargs["df_raw"], signal=1),
    )
...
def test_runtime_order_poll_loop_runs_without_waiting_for_next_bar(tmp_path: Path, monkeypatch) -> None:
    bootstrap_open, bootstrap_close, _ = _runtime_timestamps()
    broker = FakeRuntimeBroker(position_amts=[1.0])
    poll_calls: list[str] = []

    class WaitingDataService(FakeDataService):
        async def stream_closed(self, stop_event: asyncio.Event):
            if False:
                yield _live_bar(pd.Timestamp.now(tz="UTC"))
            await stop_event.wait()

    async def _poll_and_stop(self: TraderService) -> None:
        poll_calls.append("called")
        self.stop_event.set()

    def _flatten_ok(self: TraderService, *, reason: str) -> tuple[bool, str]:
        _ = reason
        return True, "flattened"

    monkeypatch.setattr("forward.live_testnet.BinanceFuturesTestnetBroker", lambda cfg: broker)
    monkeypatch.setattr("forward.live_testnet.DataService", WaitingDataService)
    monkeypatch.setattr(
        "forward.live_testnet.fetch_recent_klines_df",
        lambda **kwargs: (
            _bootstrap_df(bootstrap_open),
    ...
    monkeypatch.setattr(TraderService, "poll_open_orders", _poll_and_stop)
    monkeypatch.setattr(TraderService, "_emergency_flatten", _flatten_ok)
    monkeypatch.setenv("HEARTBEAT_PATH", str(tmp_path / "heartbeat"))

    run_dir, cfg, ft_cfg = _run_cfg(tmp_path)
    ft_cfg["testnet"]["poll_interval_seconds"] = 0.01
...
def test_runtime_reconciliation_reuses_single_position_snapshot_per_bar(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "forward.live_testnet.build_signals",
        lambda **kwargs: _build_strategy_result(kwargs["df_raw"], signal=0),
    )
    monkeypatch.setenv("HEARTBEAT_PATH", str(tmp_path / "heartbeat"))
