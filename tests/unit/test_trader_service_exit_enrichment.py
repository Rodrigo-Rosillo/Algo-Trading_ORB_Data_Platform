...
def test_exit_row_enriched_from_algo_actual_fields(tmp_path: Path) -> None:
    db_path = tmp_path / "state.db"
    emitted: list[dict[str, Any]] = []
    with SQLiteStateStore(db_path=db_path) as store:
        state = store.load_state()
        _seed_open_position(state, side="LONG", qty=0.58, entry_price=85.94, tp_order_id=2006, sl_order_id=None)
        store.save_state(state)
    ...
    exit_row = _latest_exit_row(db_path)
    fee = (85.94 * 0.58 + 87.67 * 0.58) * 0.0005
    expected = (87.67 - 85.94) * 0.58 - fee
    assert float(exit_row["price"]) == pytest.approx(87.67)
    assert float(exit_row["qty"]) == pytest.approx(0.58)
    assert float(exit_row["realized_pnl"]) == pytest.approx(expected)
