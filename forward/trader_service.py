...
from execution_specs import ExecutionSpec, get_execution_spec, required_orb_fields, resolve_execution_plan
...
def _order_avg_price(resp: Any) -> float:
    if isinstance(resp, dict):
        for key in ("avgPrice", "actualPrice", "price"):
            if resp.get(key) not in (None, ""):
                return _float(resp.get(key), 0.0)
    return 0.0


def _order_exec_qty(resp: Any) -> float:
    if isinstance(resp, dict):
        for key in ("executedQty", "actualQty", "origQty"):
            if resp.get(key) not in (None, ""):
                return _float(resp.get(key), 0.0)
    return 0.0
...
class TraderService:
    def __init__(
        ...
        execution_specs: Optional[Mapping[str, ExecutionSpec]] = None,
    ) -> None:
        self.broker = broker
        self.store = store
        self.state = state
        ...
        self.execution_specs = dict(execution_specs or {})
    ...
    async def maybe_place_trade_from_signal(self, bar_open_time: pd.Timestamp, row: pd.Series) -> None:
        ...
        execution_spec = get_execution_spec(self.execution_specs, signal_type)
        ...
        missing_field = ""
        for field_name in required_orb_fields(execution_spec):
