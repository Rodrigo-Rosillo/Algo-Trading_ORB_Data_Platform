...
def _extract_strategy_result(build_result: Any) -> StrategyBuildResult:
    if isinstance(build_result, StrategyBuildResult):
        return build_result
    raise TypeError("build_signals must return a core.strategy_plugin.StrategyBuildResult")
...
async def _poll_open_orders_loop(
    *,
    trader_service: TraderService,
    stop_event: asyncio.Event,
    poll_interval_seconds: float,
    emit_runtime_event: Callable[..., None],
) -> None:
    # Clamp to a small positive interval so misconfigured zero/negative values
    # do not create a busy loop.
    interval_seconds = max(0.01, float(poll_interval_seconds))
    while not stop_event.is_set():
        if await _wait_for_stop_or_timeout(stop_event, interval_seconds):
            break
        try:
            await trader_service.poll_open_orders()
        except Exception as e:
            emit_runtime_event(
                "ORDER_POLL_LOOP_FAILED",
                stage="runtime_poll",
                error=str(e),
            )
...
async def run_live_testnet(
    *,
    ...
    with SQLiteStateStore(db_path=db_path, events_path=events_path) as store:
        ...
        bootstrap_strategy = _extract_strategy_result(
            build_signals(
                df_raw=df_raw,
        ...
        trader_service = TraderService(
            ...
            execution_specs=bootstrap_strategy.execution_specs,
        ...
        order_poller = asyncio.create_task(
            _poll_open_orders_loop(
        ...
        try:
            async for bar in data_service.stream_closed(stop_event):
                ...
                trader_service.execution_specs = dict(strategy_result.execution_specs)
